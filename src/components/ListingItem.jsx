...
export default function ListingItem({listing, id, onEdit, onDelete}) {
  return (<>
     <li className='relative bg-white flex flex-col justify-between items-center shadow-md hover:shadow-xl rounded-md overflow-hidden transition-shadow duration-150 m-[10px]'>
         <Link className='content'  to={`/categories/${listing.type}/${id}`} >
         ...
         <p className='font-semibold text-sm mb-[2px] text-gray-600 truncate'>{listing.address}</p>
            ...
            <p className='font-semibold m-0 text-xg text-[#457b9d] truncate'>
                {listing.name}
            </p>
            <p className='text-[#457b9d] mt-2 font-semibold '>${listing.offer ? listing.discountedPrice .toString()
              ...
              : listing.regularPrice
                  ...
                  <div className='flex item-center space-x-1'>
                    <p className='font-bold text-xs'>{listing.bedrooms > 1 ? `${listing.bedrooms} Beds` : `${listing.bedrooms} Bed` }</p>
                  </div>
         ...
         {onEdit && (<MdEdit className='absolute bottom-2 right-8 h-[14px] cursor-pointer '  onClick={()=> onEdit(listing.id)} />)}
